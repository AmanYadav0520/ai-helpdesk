...
- **Frontend**: React + TypeScript + Vite (`apps/web`, port 3000) + shadcn/ui
- **Backend**: Express + TypeScript + Bun (`apps/server`, port 3001)
- **Database**: PostgreSQL with Prisma ORM
- **Auth**: Better Auth (email/password, database sessions)
- **User roles**: `admin` and `agent`
- **Sign-up is disabled** — users are seeded via `apps/server/prisma/seed.ts`
- No Vite proxy — `apps/web` and `apps/server` talk over plain cross-origin HTTP (`fetch`); the client reads the API URL from `VITE_API_URL`
- Use `@/` path alias for imports in `apps/web` (maps to `./src/`)
- AI integration and email ingestion are **not built yet** — check `PROJECT.md`, `tech-stack.md`, and `IMPLEMENTATION_PLAN.md` for current scope before assuming a flow exists
...
- **Config**: Root `playwright.config.ts` — dual `webServer` starts both `apps/server` and `apps/web` automatically
- **Test database**: `helpdesk_test` (isolated from dev `helpdesk` DB), configured in `apps/server/.env.test`
- **Ports**: Test server on 3002, test client on 5174 (dev uses 3001/3000)
...
- Since sign-up is disabled, create an auth helper that logs in with seeded user credentials (see `e2e/fixtures/auth.ts`).
- Use `storageState` to persist auth sessions across tests where appropriate.
- Create separate auth states for `admin` and `agent` roles — only `admin` is currently seeded, so add `agent` fixtures once an agent user exists in the seed script.
- Check `apps/server/prisma/seed.ts` to find seeded user credentials.
...
1. **Authentication flows**: Login, logout, session persistence, role-based redirects — currently the only implemented feature area
2. **Core CRUD operations**: Creating, reading, updating tickets — not built yet; add coverage once ticket routes/pages exist
3. **Role-based access**: Admin vs agent permissions, protected routes, admin-only routes
4. **AI features**: Ticket classification, auto-responses (mock AI responses for deterministic tests) — not built yet
...
4. Run the tests using `bun run test:e2e` to verify they pass (if the dev servers are available).
